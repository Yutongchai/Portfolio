  ...
  const footerSections = [
    {
      title: "Company",
      links: [
        { label: "Our Value", path: "/personal-story-section#beliefs-values" },
        { label: "Contact us", path: "/connection-hub#contact-methods" }
      ]
    },
    {
      title: "Services",
    ...
    <footer className="bg-[#153462] text-white font-light">
      ...
      <div className="max-w-7xl mx-auto px-8 pb-16">
                ...
                <ul className="space-y-4">
                  {section.links.map((linkObj) => (
                    <li key={linkObj.label}>
                      <a href={linkObj.path} className="text-white hover:text-[#79989f] transition-colors">
                        {linkObj.label}
                      </a>
                    </li>
                  ))}
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between">
          <span className="tracking-[0.3em] text-[10px] md:text-xs uppercase opacity-50 text-center md:text-left mb-4 md:mb-0">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              className="flex items-center gap-2 text-white/70 hover:text-[#f68921] transition-colors group"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
