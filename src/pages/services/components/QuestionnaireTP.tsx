...
const QuestionnaireTP = () => {
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 2) {
      return !!(formData.noOfPax && formData.duration && formData.trainingTypes.length > 0 && formData.eventMonth && formData.eventYear && formData.budget && formData.location);
    }
    return true;
  };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white border-4 border-[#153462] rounded-[2rem] md:rounded-[3rem] md:shadow-[12px_12px_0px_0px_#153462] p-5 md:p-12">
        ...
        <div className="flex justify-between gap-4 mb-3 md:mb-6 border-b-4 border-[#153462] pb-4 md:pb-6">
          {[1, 2, 3].map((num) => (
            <div key={num} className="flex items-center gap-2 flex-1 justify-center">
              <div className={`w-8 h-8 flex items-center justify-center font-black border-2 border-[#153462] transition-all ${
                step === num ? 'bg-[#f68921] text-white scale-110' : 
                step > num ? 'bg-[#153462] text-white' : 'bg-gray-100 text-[#153462]'
              }`}>
                {num}
              </div>
              <span className={`text-[10px] font-black uppercase hidden md:block ${
                step === num ? 'text-[#f68921]' : 'text-slate-400'
              }`}>
                {num === 1 ? 'Company' : num === 2 ? 'Training Details' : 'Preferences'}
              </span>
            </div>
          ))}
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
            <h3 className="text-2xl font-black text-[#153462] mb-4 md:mb-6 uppercase italic">Step 2: Training Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={inputWrapperStyle}>
              ...
              <div className="flex flex-wrap gap-3">
                {["Half day", "Full day"].map((item) => (
                  <button
                    ...
                    className={`px-4 py-2 border-2 border-[#153462] font-black uppercase text-xs transition-all ${
                      formData.duration === item ? "bg-[#153462] text-white -translate-y-1 shadow-[4px_4px_0px_0px_#fcb22f]" : "bg-white text-[#153462]"
                    }`}
                  >
                    {item}
                  </button>
            ...
            <div className={inputWrapperStyle}>
              <label className={labelStyle}>Types of Training *</label>
                ...
                {["Mental Health & Wellbeing", "Leadership & Management", "Personal Development & Soft Skills", "Others"].map((type) => (
                  <label key={type} className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative">
                      ...
                      />
                      <div className={`w-6 h-6 border-2 border-[#153462] transition-all shadow-[2px_2px_0px_0px_#153462] ${
                        formData.trainingTypes.includes(type) ? "bg-[#f68921]" : "bg-white"
                      }`} />
                    </div>
                    <span className="font-bold uppercase text-xs text-[#153462]">{type}</span>
                  </label>
                ))}
              </div>
              {fieldErrors.trainingTypes && <div className="text-sm text-red-600 mt-2">{fieldErrors.trainingTypes}</div>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={inputWrapperStyle}>
                <label className={labelStyle}>Training Month *</label>
                <Select
                  name="eventMonth"
              ...
              <div className={inputWrapperStyle}>
                <label className={labelStyle}>Training Year *</label>
                <Select
                  name="eventYear"
                  className="border-4 border-[#153462] font-bold shadow-[4px_4px_0px_0px_#153462]"
        ...
        {step === 3 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
                ...
                {["Yes", "No"].map((opt) => (
                  <button
                    ...
                    className={`flex-1 py-3 border-2 border-[#153462] font-black uppercase transition-all ${
                      formData.hrdc === opt ? 'bg-[#153462] text-white shadow-[4px_4px_0px_0px_#fcb22f]' : 'bg-white text-[#153462]'
                    }`}
                  >
                    {opt}
                  </button>
            ...
            <div className={inputWrapperStyle}>
              <label className={labelStyle}>Language to Conduct *</label>
                ...
                {["English", "Mandarin", "BM", "Others"].map((lang) => (
                  <label key={lang} className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative">
                      ...
                      />
                      <div className={`w-6 h-6 border-2 border-[#153462] transition-all shadow-[2px_2px_0px_0px_#153462] ${
                        formData.languages.includes(lang) ? "bg-[#f68921]" : "bg-white"
                      }`} />
                    </div>
            ...
            <div className="mb-6">
              <label className="flex items-start gap-4 cursor-pointer group">
                ...
                />
                <div className={`mt-1 min-w-[20px] h-5 w-5 border-2 border-[#153462] shadow-[2px_2px_0px_0px_#153462] transition-all ${
                  privacyConsent ? "bg-[#153462]" : "bg-white"
                }`} />
