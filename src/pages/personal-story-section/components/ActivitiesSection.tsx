...
const ActivitiesSection = () => {
  ...
  const fetchActivities = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/activities?order=featured.desc,created_at.desc`, {
        headers: {
      ...
      >
        ...
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-background rounded-3xl p-8 shadow-xl animate-pulse">
