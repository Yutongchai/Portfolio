...
app.get('/api/projects', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('projects')
      .select(`*, project_gallery(url, alt), project_types(type_key, id as type_id)`)
...
app.post('/api/projects', async (req, res) => {
  ...
  try {
    ...
    if (projectId) {
      ...
      const { data: inserted, error: insertErr } = await supabase.from('projects').insert([payload]).select().single();
    ...
    if (projectId && Array.isArray(payload.gallery)) {
      ...
      if (payload.gallery.length) {
        const rows = payload.gallery.map((g) => ({ project_id: projectId, url: g.url, alt: g.alt || null }));
        const { error: insErr } = await supabase.from('project_gallery').insert(rows);
...
app.post('/api/project-types', async (req, res) => {
  ...
  try {
    const { data, error } = await supabase.from('project_types').insert([{ type_key, description: description || null }]).select().single();
    if (error) return res.status(500).json({ error: error.message });
    return res.json(data);
  } catch (err) {
    console.error(err);
